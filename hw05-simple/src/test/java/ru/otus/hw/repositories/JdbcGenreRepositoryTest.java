...
class JdbcGenreRepositoryTest {
    ...
    void shouldReturnCorrectGenreById(Genre expectedGenre) {
        var actualGenre = repo.findById(expectedGenre.getId());
        assertThat(actualGenre).isPresent().contains(expectedGenre);
    }

    @DisplayName("должен возвращать пустой объект по несуществующему ID")
    @ParameterizedTest
    @ValueSource(longs = {-1, 1000, -1000})
    void shouldReturnEmptyGenreByIncorrectId(long incorrectId) {
        var actualGenre = repo.findById(incorrectId);
