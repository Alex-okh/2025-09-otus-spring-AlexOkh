...
class ModelsCommonTest {
    ...
    void shouldBeNoOneToOneRelationshipsInModelClasses(Class<?> entityClass) {
        ...
        assertThat(oneToOneRelationshipExists).withFailMessage("В доменной модели ДЗ не предусмотрены связи OneToOne")
                                              .isFalse();
    }
    ...
    void shouldBeNoEagerRelationshipsInModelClasses(Class<?> entityClass) {
        ...
        assertThat(eagerFetchExists).withFailMessage("Лучше все связи сделать LAZY")
                                    .isFalse();
    }
    ...
    void shouldMappedForBidirectionalRelationshipsInModelClasses(Class<?> entityClass) {
        ...
        var hasBidirectionalRelationshipsWithoutMappedBy = relationsEntries.entrySet()
                                                                           .stream()
                                                                           .anyMatch(relationEntry -> {
                                                                               var reverseRelations = findAllRelationsEntry(
                                                                                       relationEntry.getKey());
                                                                               var reverseRelationField = reverseRelations.get(
                                                                                       entityClass);
                                                                               if (isNull(reverseRelationField)) {
                                                                                   return false;
                                                                               }
                                                                               var relationFieldName = relationEntry.getValue()
                                                                                                                    .getName();
                                                                               ...
                                                                               var reverseMappedByValue = getRelationAnnotationArgumentValue(
                                                                                       reverseRelationField, "mappedBy",
                                                                                       String.class);

                                                                               return !reverseRelationFieldName.equals(
                                                                                       mappedByValue) &&
                                                                                      !relationFieldName.equals(
                                                                                              reverseMappedByValue);
                                                                           });
        assertThat(hasBidirectionalRelationshipsWithoutMappedBy).withFailMessage(
                                                                        "Двунаправленные связи должны быть настроены с помощью mappedBy")
                                                                .isFalse();
    ...
    private <T> T getRelationAnnotationArgumentValue(Field field, String argumentName, Class<T> returnType) {
        return Arrays.stream(field.getAnnotations())
                     .flatMap(a -> Arrays.stream(a.getClass()
                                                  .getDeclaredMethods())
                                         .map(m -> Map.entry(m, a)))
                     .filter(e -> e.getKey()
                                   .getName()
    ...
    private Class<?> fieldToClass(Field field) {
        var className = field.getType()
                             .getName();
        if (Collection.class.isAssignableFrom(field.getType())) {
            className = field.getGenericType()
                             .getTypeName()
                             .split("<")[1].split(">")[0];
